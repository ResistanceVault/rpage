...
{
    BltBitMap(main_screen->bitmaps[main_screen->physical], source_x, source_y, dest_bitmap, 0, 0, width, height, 0xC0, 0xFF, NULL);
}
...
{
    BltBitMap(source_bitmap, source_x, source_y, main_screen->bitmaps[main_screen->physical], x, y, width, height, 0xC0, bit_mask, NULL);
}
...
void __inline rpage_video_set_palette_to_black(short first_color, short last_color)
{
	...
	for (loop = first_color; loop <= last_color; loop++)
		main_screen->palettes[main_screen->physical][loop] = 0x0;
}
