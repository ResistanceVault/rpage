...
UBYTE *load_pak_locale_to_array(char *text_array[], UWORD array_size, char *filename)
{
    ...
    if ((fileHandle = Open(filename, MODE_OLDFILE)))
    {
		...
		else
		{
            ...
            unpacked_block = (UBYTE *)malloc(unpacked_block_size);
#ifdef DEBUG_MACROS
...
#endif
                packed_block = (UBYTE *)malloc(packed_block_size);
...
#endif
                    str_ptr += 2;
                    text_array[i] = str_ptr; // (char *)malloc(str_len);
            ...
            {
                printf("cannot found tag 'SIZE', found %s instead!\n", tag);
            }
